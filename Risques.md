...
Un risque est un événement qui, s'il se réalise, aura un impact.

De manière générale, on peut placer les risque dans une matrice :
`Risque = Probabilité(%) x Impacte(1 à 5)`

On peut aussi calculer le coût d'un risque :
`Coût risque = Proba(%) x Impacte(coût en cas de réalisation)`

En fonction peut choisir :
- accepter le risque
- mitiger le risque
- contourner le risque 
- transfert le risque 

Si on cherche à le mitiger, on peut proposer des pistes d'atténuation (engagé un coût pour réduire la proba) et si elles sont intéressantes (formule 

## Fissure murs chambre 1er et Fissure sol 1er
...
Les risques sont :
 - la degradation augmente 
 - la déstabilisation d'un mur
